...
export class PhotoAccess {
  ...
  async savePhoto(userPhotoDto: UserPhoto): Promise<UserPhoto> {
    try {
      logger.info('Saving a photo for user', userPhotoDto)
      ...
      await this.docClient
        ...
        .promise()
      logger.info('Saving a photo for user successfully', userPhotoDto)

      return userPhotoDto
    } catch (error) {
      logger.error('Failed to save a photo for user', error, userPhotoDto)
      return null
    }
  }
